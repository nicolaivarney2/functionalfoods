-- Tilbudsavis-kilder (Tjek/Squid) skal tælle som tilbud SELV uden en struktureret
-- førpris. Leaflet-tilbud (Løvbjerg, Lidl, MENY, Spar, Kvickly, SuperBrugsen,
-- Brugsen, 365 Discount, ABC Lavpris) har sjældent en `normal_price`, men varen
-- ER et tilbud så længe den er i sit gyldighedsvindue.
--
-- Tidligere krævede både get_food_offers_v2 og get_product_counts_v2 en bevist
-- rabat (normal_price > current_price), hvilket gjorde at ALLE Tjek-kæder viste
-- 0 tilbud på /dagligvarer. Her udvider vi "tilbud"-prædikatet med en OR-gren for
-- sale-only kilder (source LIKE 'tjek%'). Ingen data ændres.

-- Sorterings-index for tilbudsavis-tilbud (normal_price er typisk NULL her, så de
-- dækkes ikke af idx_product_offers_fooddata_offer_sort).
CREATE INDEX IF NOT EXISTS idx_product_offers_saleonly_sort
  ON public.product_offers (discount_percentage DESC NULLS LAST, current_price ASC)
  WHERE is_available = true
    AND source IS DISTINCT FROM 'goma'
    AND source LIKE 'tjek%';
