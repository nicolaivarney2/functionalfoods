-- /dagligvarer performance (jul 2026): kategori-RPC, udvidet tilbuds-RPC, partial indexes.
-- Bygger på get_food_offers_v2 / Goma-primary fra 20260702120000_goma_primary_no_tjek.sql.

CREATE INDEX IF NOT EXISTS idx_product_offers_food_offer_sort
  ON public.product_offers (discount_percentage DESC NULLS LAST, current_price ASC)
  WHERE is_available = true
    AND normal_price IS NOT NULL
    AND normal_price > current_price + 0.01;

CREATE INDEX IF NOT EXISTS idx_products_department_lower
  ON public.products (lower(department));

CREATE INDEX IF NOT EXISTS idx_products_category_lower
  ON public.products (lower(category));

-- Ét DB-kald for kategori-filter (alias merge) — erstat mange app-side ilike queries.
DROP FUNCTION IF EXISTS public.find_product_ids_for_categories(text[]);

CREATE OR REPLACE FUNCTION public.find_product_ids_for_categories(p_patterns text[])
RETURNS text[]
LANGUAGE sql
STABLE
SECURITY INVOKER
...
AS $$
  ...
  WHERE p_patterns IS NOT NULL
    AND cardinality(p_patterns) > 0
    AND EXISTS (
      SELECT 1
      FROM unnest(p_patterns) AS pat(pattern)
      WHERE p.department ILIKE pat.pattern
         OR p.category ILIKE pat.pattern
    );
$$;

GRANT EXECUTE ON FUNCTION public.find_product_ids_for_categories(text[]) TO anon, authenticated, service_role;
...
DROP FUNCTION IF EXISTS public.get_food_offers_v2(boolean, integer, integer, text[], boolean, boolean, text[]);
DROP FUNCTION IF EXISTS public.get_food_offers_v2(boolean, integer, integer, text[], boolean, boolean, text[], text);
...
GRANT EXECUTE ON FUNCTION public.get_food_offers_v2(boolean, integer, integer, text[], boolean, boolean, text[], text) TO anon, authenticated, service_role;
