-- Tjek/Squid-tilbud (tilbudsavis-kilder) har INGEN food-taksonomi: deres
-- `department` er forhandlernavnet (fx "Lidl", "MENY"), ikke en rigtig
-- fødevarekategori. Derfor faldt de altid ud af /dagligvarer-filteret, som
-- gatede på `prod.department IN (food_departments)` — uanset 20260625150000's
-- fix af selve tilbuds-prædikatet.
--
-- Fix: udvid food-gaten med en OR-gren for tilbudsavis-kilder (source LIKE
-- 'tjek%'), så hele ugens tilbudsavis vises for de kæder. Ingen ny kolonne
-- kræves (den denormaliserede is_food findes ikke i prod), ingen data ændres.
