-- Goma genaktiveret som primær tilbudskilde for offers-only kæder (jun 2026).
-- Fjern sunset-filter der skjulte source=goma og efterlod gamle Tjek-tilbudsavis-billeder.
--
-- p_goma_primary=false bevarer legacy-adfærd (skjul goma) til GOMA_SIMULATE_GONE / sunset.
    ...
    WHERE po.is_available = true
      AND (
        ...
        OR (
          ...
          AND (
            po.source = 'goma'
            OR po.source NOT LIKE 'tjek%'
            OR po.store_id IN ('netto', 'bilka', 'foetex', 'rema-1000')
          )
        )
      )
      AND (
