-- /dagligvarer: kategori-filter i get_food_offers_v2 + trigram-søgeindeks.
-- Bygger på 20260702130000_dagligvarer_offers_performance.sql.

CREATE SCHEMA IF NOT EXISTS extensions;
CREATE EXTENSION IF NOT EXISTS pg_trgm WITH SCHEMA extensions;

CREATE INDEX IF NOT EXISTS idx_products_name_generic_trgm
  ON public.products USING gin (name_generic extensions.gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_products_brand_trgm
  ON public.products USING gin (brand extensions.gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_product_offers_name_store_trgm
  ON public.product_offers USING gin (name_store extensions.gin_trgm_ops);
