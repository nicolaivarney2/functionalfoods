-- Speeds up incremental fooddata → FF price_history import.
-- Without this index, PostgREST "snapshot_date >= X ORDER BY snapshot_date, id"
-- scans the full table and hits statement_timeout (~8s).
--
-- The RPC uses the same index with a 60s timeout so daily import can page
-- through 3 days of snapshots without 2000+ per-product roundtrips.
