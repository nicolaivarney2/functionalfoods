...
type Props = {
  ...
  /** mørk baggrund (onboarding) vs. lys (login modal) */
  variant?: 'dark' | 'light'
}
...
export default function OAuthProviderButtons({
  ...
  const startOAuth = async (provider: OAuthProvider) => {
    ...
    if (error) {
      ...
      onError?.(error.message || 'Kunne ikke starte login.')
    }
  }
      ...
      {providers.map((provider) => {
        ...
        return (
          <button
            ...
            className={`flex w-full items-center justify-center gap-3 rounded-xl px-4 py-3.5 text-sm font-semibold transition disabled:cursor-not-allowed disabled:opacity-60 ${
              isApple
                ? isDark
                  ? 'bg-black text-white hover:bg-black/90'
                  : 'bg-black text-white hover:bg-gray-900'
                : isDark
                  ? 'bg-white text-gray-900 ring-1 ring-white/20 hover:bg-gray-50'
          ...
          >
            ...
            <span>{loading ? 'Sender dig videre…' : actionLabel(provider, mode, tier)}</span>
          </button>
        )
      })}
    </div>
