...
export async function POST(request: NextRequest) {
  try {
    ...
    const systemPrompt = createGLP1SystemPrompt(existingTitles)
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Generer en ny GLP-1 opskrift der er unik og ikke ligner eksisterende opskrifter. Fokuser på protein, fibre og sunde fedtstoffer der stimulerer GLP-1 naturligt.`
          }
        ],
        temperature: 0.8,
        max_tokens: 2000
    ...
    const recipe = parseGeneratedRecipe(recipeContent, 'glp-1')
...
function createGLP1SystemPrompt(existingTitles: string[]): string {
...
- Protein i hvert måltid (æg, kylling, fisk, græsk yoghurt)
- Fiberrige grøntsager og bælgfrugter (broccoli, linser, havre)
- Sunde fedtstoffer (avocado, olivenolie, nødder)
- Ingen flydende kalorier og snacks
- 2-3 mættende måltider om dagen
- Fokus på solide måltider der stimulerer GLP-1 naturligt

OPPSKRIFT FORMAT (returner kun JSON):
{
  "title": "GLP-1 opskrift titel",
  "description": "Kort beskrivelse med fokus på protein, fibre og sunde fedtstoffer der stimulerer GLP-1",
  "ingredients": [
    {
...
- Protein: æg, kylling, fisk, græsk yoghurt, kød, bælgfrugter
- Fiberrige grøntsager: broccoli, spinat, kål, linser, bønner, havre
- Sunde fedtstoffer: avocado, olivenolie, nødder, frø
- Bælgfrugter: linser, bønner, kikærter (stimulerer GLP-1)
...
- Snacks: chips, kager, slik
- Processerede fødevarer med tilsat sukker
- Fokus på solide, mættende måltider`
}
